...
Design system for outbound communications agents send to Oliver —
emails/newsletters, docs, and Artifacts. Separate concern from the rest
of this repo (which is the `@n3wth/ui` app component library); this folder
shares its canonical palette (`../src/theme.css`) but has no runtime
dependency on it — everything here is static assets and templates consumed
directly by an agent, not imported code.

## Layout

- `tokens.md` — literal hex values (email/docs can't use CSS variables)
- `assets/` — identity marks (`hermes`, `hop`, `finance`), each an SVG +
  PNG, self-contained dark tile (own bg baked in, like an app icon) — no
  separate light variant needed
