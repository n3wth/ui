...
- Colors: `../tokens.md` (`#08090b` bg, `#f2f3f5` ink, etc.) — the same
  ...
  a one-off Artifact.
- Favicon / logo mark: `../assets/claude-mark.svg` when an Artifact wants
  one — pass it via the Artifact tool's `favicon` mechanism or inline it;
  don't use a generic emoji or unrelated icon.
...
## When it's Claude building the Artifact

Since these are Artifacts *I* build for Oliver, the sender mark is
`claude-mark.svg` by default — the Claude identity, not a generic "n3wth"
badge.
